...
export const authMiddleware = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        req.userId = decoded.id // attach the user's ID to the request object, now any route handler after this middleware can access req.userId
        next() // all good! pass to the next checkpoint (REQUIRED FOR ALL MIDDLEWARES)
    } catch {
