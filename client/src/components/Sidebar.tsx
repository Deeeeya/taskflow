...
export const Sidebar = ({ projects, onNewProject, activeProjectId, onSelectProject, isCollapsed, onToggleCollapse }: SidebarProps) => {
    ...
    return (
        <aside className={`fixed left-0 top-0 h-full flex flex-col border-r bg-background transition-all duration-300 ${isCollapsed ? 'w-10' : 'w-64'}`}>
            {isCollapsed && (
                <div className="p-2 flex justify-center">
                    <Button variant="ghost" onClick={onToggleCollapse} className="px-2 py-1.5 rounded-md">
            ...
            {!isCollapsed && (
                <>
                    ...
                    <div className="p-3 border-b flex items-center gap-2">
                        ...
                        <Button variant="ghost" onClick={onToggleCollapse} className="ml-auto px-2 py-1.5 rounded-md">
                    ...
                    <nav className="p-2 flex flex-col gap-1">
                        ...
                        <Button className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-sm font-normal ${isCollapsed ? 'justify-center' : 'justify-start'}`} variant="ghost">
                    ...
                    <div className="p-2 flex flex-col gap-1">
                        ...
                        <div className="flex items-center pl-2 pb-1">
                            <p className={`text-xs text-muted-foreground uppercase font-medium pb-1 ${isCollapsed ? 'hidden' : 'block'}`}>MY PROJECTS</p>
                        ...
                        {projects.map((project) => (
                            <Button key={project.id} onClick={() => onSelectProject(project.id)} variant="ghost" className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-sm ${isCollapsed ? 'justify-center' : 'justify-start'}
                    ...
                    {/* General Section */}
                    <div className="p-2 flex flex-col gap-1">
                        <p className={`text-xs text-muted-foreground uppercase font-medium pl-2 pb-1 ${isCollapsed ? 'hidden' : 'block'}`}>GENERAL</p>
                        <Button variant="ghost" className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-xs ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
                            <Settings className="w-4 h-4 shrink-0" />
                            <span className={isCollapsed ? 'hidden' : 'block'}>Settings</span>
                        </Button>
                        <Button variant="ghost" className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-xs ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
                            <Trash2 className="w-4 h-4 shrink-0" />
                            <span className={isCollapsed ? 'hidden' : 'block'}>Trash</span>
                        </Button>
                    </div>

                    {/* Bottom Section */}
                    <div className={`mt-auto p-2 border-t flex items-center gap-2 ${isCollapsed ? 'justify-center' : ''}`}>
                        {/* Theme Toggle Button */}
                        <Button variant="ghost" size="icon" onClick={toggleTheme}>
                            {theme == 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
                        </Button>
                        {/* Logout Button */}
                        <Button variant="ghost" size="icon" onClick={logout} className={isCollapsed ? 'hidden' : 'block'} >
